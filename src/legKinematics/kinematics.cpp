...
{
    Eigen::Vector3d out;
    fk_pf_pos(q.data(), rho_opt.data(), rho_fix.data(), out.data());
    return out;
}
...
{
    Eigen::Matrix3d mtx;
    d_fk_dt(q.data(), rho_opt.data(), rho_fix.data(), mtx.data());
    return mtx;
}

// 位置
void Kinematics::fk_pf_pos(const double in1[3], const double in2[RHO_OPT_SIZE], const double in3[RHO_FIX_SIZE], double p_bf[3])
{
...
// 速度
void Kinematics::d_fk_dt(const double in1[3], const double in2[RHO_OPT_SIZE], const double in3[RHO_FIX_SIZE], double jacobian[9])
{
    // double lc = in2[0];
    // double jacobian_tmp;
    // double t10;
